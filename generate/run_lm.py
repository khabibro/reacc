...
def eval_line_completion(args, model, tokenizer, file_type='test', load_file=None, res_file=None, save_name=None):
    ...
    pred_file = open(save_name, "a" if resume_count else "w") if save_name else None
    for step, (inputs, gt) in enumerate(test_dataloader):
        if step < resume_count:
            continue
        ...
        with torch.no_grad():
            ...
            p = []
            zero = torch.zeros(1, dtype=torch.long, device=args.device)
    ...
    total = max(len(preds), resume_count)
    logger.warning(f"Test {total} samples")
    logger.warning(f"Edit sim: {edit_sim/total}, EM: {em/total}")
